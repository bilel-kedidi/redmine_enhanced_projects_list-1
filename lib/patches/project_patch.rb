...
module  Patches
  module ProjectPatch
    def self.included(base)
      ...
      base.class_eval do
        ...
        # end
        class<< self
          alias_method_chain :project_tree, :new_order_old unless Redmine::VERSION.to_s > '3.4.0'
          alias_method_chain :project_tree, :new_order if Redmine::VERSION.to_s > '3.4.0'
  ...
  module ClassMethods
    def project_tree_with_new_order(projects, options={}, &block)
      return project_tree_without_new_order(projects, options, &block) if projects.blank? or  !projects.first.attributes.has_key?('order_lft') or !projects.first.attributes.has_key?( 'order_rgt' )
      ancestors = []
    ...
    def project_tree_with_new_order_old(projects, &block)
      return project_tree_without_new_order_old(projects, &block) if projects.blank? or  !projects.first.attributes.has_key?('order_lft') or !projects.first.attributes.has_key?( 'order_rgt' )
      ancestors = []
      # projects = get_all_projects(projects, order_desc, is_closed)
      projects = Project.get_new_tree(projects)
      projects.each do |project|
